...
int memory_allocation(char** string_memory_allocation, char** for_memory_leak_string, int len) {
    ...
    {
        if (for_memory_leak_string != NULL)
        {
            free_memory(for_memory_leak_string, len);
        };
...
int overfscanf(FILE* file, char* format, ...)
{
    ...
    if (memory_allocation(buffer_string, NULL, len))
    {
        return MEMORY_RECEIVED_DATA_ALLOCATION_ERROR;
    }
    ...
    if (memory_allocation(specification_buffer_string, buffer_string, len))
    {
        return MEMORY_SPECIFICATION_DATA_ALLOCATION_ERROR;
    }
